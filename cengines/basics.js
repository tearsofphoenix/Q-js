  ...
  allocateQubit(dirty = false) {
    ...
    if (dirty) {
      if (this.isMetaTagSupported(DirtyQubitTag)) {
        ...
        this.main.dirtyQubits.push(qubit.id)
      }
    }
...
ValueError: Qubit already deallocated. Caller likely has a bug.
  ...
  deallocateQubit(qubit) {
    ...
    this.send([new Command(this,
      Deallocate,
      [new Qureg([qubit])],
      is_dirty ? [new DirtyQubitTag()] : [])])
  }
...
returns True.
  ...
  isMetaTagSupported(metaTag) {
    ...
    try {
      while (true) {
        if (engine.isMetaTagHandler(metaTag)) {
          return true
        }
        engine = engine.next
      }
