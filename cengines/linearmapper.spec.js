...
describe('linearmapper test', () => {
  ...
  //   }
  //   LinearMapper._process_two_qubit_gate(mapper.num_qubits,
  //     mapper.cyclic,
  //     4,
  //     6,
  //     active_qubits,
  ...
  //     const new_mapping = LinearMapper._return_new_mapping_from_segments(
  //       mapper.num_qubits,
  //       segments,
  //       allocated_qubits,
  //       mapper.currentMapping
  ...
  //       }
  //     })
  //     const swaps = mapper._odd_even_transposition_sort_swaps(old_map, new_map)
...
// mapper._stored_commands = [cmd0, cmd1, cmd2, cmd3]
// // Following chain 0 <-> 2 <-> 3 <-> 1
// mapper.currentMapping = {0: 0, 2: 1, 3: 2, 1: 3}
// mapper._send_possible_commands()
// expect(len(backend.receivedCommands)).to.equal(2
// expect((backend.receivedCommands[0]).to.equal(new Command(null, CNOT,
//     qubits=([qb0],),
//     controls=[qb3]))
// expect(backend.receivedCommands[1]).to.equal(new Command(null, X, tuple([qb0],))
// // Following chain 0 <-> 2 <-> 1 <-> 3
// mapper.currentMapping = {0: 0, 2: 1, 3: 3, 1: 2}
// mapper._send_possible_commands()
// expect(len(backend.receivedCommands)).to.equal(4
// expect(len(mapper._stored_commands)).to.equal(0
//
...
// expect(mapper._stored_commands).to.equal([cmd0, cmd1, cmd2, cmd3, cmd4, cmd5]
// qb3 = new BasicQubit(null, -1)
// cmd_flush = new Command(null, new FlushGate(), tuple([qb3],))
// mapper.receive([cmd_flush])
// expect(mapper._stored_commands).to.equal([]
// expect(len(backend.receivedCommands)).to.equal(7
// expect(mapper._currently_allocated_ids).to.equal(new Set([0, 2])
// expect((mapper.currentMapping).to.equal({0: 2, 2: 0} or
// mapper.currentMapping).to.equal({0: 0, 2: 2})
// cmd6 = new Command(null, X, tuple([qb0]), [qb2])
// mapper.storage = 1
// mapper.receive([cmd6])
// expect(mapper._currently_allocated_ids).to.equal(new Set([0, 2])
// expect(mapper._stored_commands).to.equal([]
// expect(len(mapper.currentMapping)).to.equal(2
// expect(0 in mapper.currentMapping
// expect(2 in mapper.currentMapping
// expect(len(backend.receivedCommands)).to.equal(11
// for cmd in backend.receivedCommands:
// print(cmd)
// expect((backend.receivedCommands[-1]).to.equal(new Command(null, X,
//     qubits=([new BasicQubit(null,
//         idx=mapper.currentMapping[qb0.id])],),
//     controls=[new BasicQubit(null,
//         idx=mapper.currentMapping[qb2.id])]))
...
// cmd_flush = new Command(null, new FlushGate(), tuple([qb2],))
...
// const cmd1 = new Command(null, Allocate,tuple([qb1],))
// const cmd2 = new Command(null, X, tuple([qb0]), [qb1])
// const cmd3 = new Command(null, gate=Deallocate, tuple([qb0],))
// const cmd4 = new Command(null, gate=Deallocate, tuple([qb1],))
// mapper.currentMapping = {0: 0, 1: 3}
// qb_flush = new BasicQubit(null, -1)
// cmd_flush = new Command(null, new FlushGate(), tuple([qb_flush],))
// mapper.receive([cmd0, cmd1, cmd2, cmd_flush])
// expect(backend.receivedCommands[0].gate).to.equal(Allocate
// expect(backend.receivedCommands[0].qubits[0][0].id).to.equal(0
// expect(backend.receivedCommands[0].tags).to.equal([LogicalQubitIDTag(0)]
// expect(backend.receivedCommands[1].gate).to.equal(Allocate
// expect(backend.receivedCommands[1].qubits[0][0].id).to.equal(3
// expect(backend.receivedCommands[1].tags).to.equal([LogicalQubitIDTag(1)]
// for cmd in backend.receivedCommands[2:]:
// if cmd.gate).to.equal(Allocate:
// expect(cmd.tags).to.equal([]
// elif cmd.gate).to.equal(Deallocate:
// expect(cmd.tags).to.equal([]
// mapped_id_for_0 = mapper.currentMapping[0]
// mapped_id_for_1 = mapper.currentMapping[1]
// mapper.receive([cmd3, cmd4, cmd_flush])
// expect(backend.receivedCommands[-3].gate).to.equal(Deallocate
// expect(backend.receivedCommands[-3].qubits[0][0].id).to.equal(mapped_id_for_0
// expect(backend.receivedCommands[-3].tags).to.equal([LogicalQubitIDTag(0)]
// expect(backend.receivedCommands[-2].gate).to.equal(Deallocate
// expect(backend.receivedCommands[-2].qubits[0][0].id).to.equal(mapped_id_for_1
// expect(backend.receivedCommands[-2].tags).to.equal([LogicalQubitIDTag(1)]
//
//
