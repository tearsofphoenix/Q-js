...
var _linearmapper = require('../cengines/linearmapper');
...
function high_level_gates(eng, cmd) {
  var g = cmd.gate;
  if (g.equal(_ops.QFT) || (0, _cycle.getInverse)(g).equal(_ops.QFT) || g.equal(_ops.Swap)) {
    return true;
  } else if (g instanceof _ops.BasicMathGate) {
    return false;
  }
  return true;
}
 ...
 */
function getEngineList(num_qubits) {
  ...
  function low_level_gates(eng, cmd) {
    ...
    (0, _assert2.default)(all_qubits.length <= 2);
    if (cmd.gate instanceof _ops.ClassicalInstructionGate) {
      // This is required to allow Measure, Allocate, Deallocate, Flush
      return true;
