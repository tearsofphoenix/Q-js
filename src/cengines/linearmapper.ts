 ...
 */
export function return_swap_depth(swaps: number[][]) {
  ...
  const values: number[] = Object.values(depth_of_qubits);
  values.push(0);
...
is mapped qubit id from 0,...,this.numQubits
...
export default class LinearMapper extends BasicMapperEngine {
  ...
  Initialize a LinearMapper compiler engine.
    ...
    @param numQubits Number of physical qubits in the linear chain
    @param cyclic If 1D chain is a cycle. Default is false.
    @param storage Number of gates to temporarily store, default is 1000
  */
  constructor(numQubits: number, cyclic: boolean = false, storage: number = 1000) {
    super()
  ...
  isAvailable(cmd: ICommand) {
    let numQubits = 0
    cmd.allQubits.forEach(qureg => numQubits += len(qureg))
    return numQubits <= 2
  }
  ...
  static returnNewMapping(numQubits: number, cyclic: boolean,
    ...
    for (let i = 0; i < stored_commands.length; ++i) {
      ...
      if (len(allocated_qubits) === numQubits && len(active_qubits) === 0) {
        break
      }
      ...
      if (len(qubit_ids) > 2 || len(qubit_ids) === 0) {
        ...
        if (len(allocated_qubits) < numQubits) {
...
possible or updates the segements such that the gate is possible.
  ...
  static _processTwoQubitGate(numQubits: number, cyclic: boolean, qubit0: number, qubit1: number,
    ...
    // the end of segement:
      ...
      // in this branch.
        ...
        neighbour_ids[qubit0].add(qubit1)
        neighbour_ids[qubit1].add(qubit0)
        if (cyclic && len(segments[0]) === numQubits) {
  ...
  */
  _oddEvenTranspositionSortSwaps(old_mapping, new_mapping) {
    ...
    const all_ids = setFromRange(this.numQubits)
  ...
  It first allocates all 0, ..., this.numQubits-1 mapped qubit ids, if
  ...
  _run() {
    ...
    const new_mapping = LinearMapper.returnNewMapping(this.numQubits,
      this.cyclic,
    ...
    if (swaps.length > 0) { // first mapping requires no swaps
      ...
      const not_needed_anymore = setDifference(setFromRange(this.numQubits), mapped_ids_used)
  ...
  */
  ...
  static _returnNewMappingFromSegments(numQubits, segments, allocated_qubits, current_mapping) {
    ...
    const previous_chain = new Array(numQubits)
    if (current_mapping) {
    ...
    const new_chain = new Array(numQubits)
