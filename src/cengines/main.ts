...
engines and adds the back-end as the last engine.
    ...
    @param backend Backend to send the compiled circuit to.
    @param engineList List of engines / backends to use
   ...
   @param verbose Either print full or compact error messages.
...
qubit.
    ...
    @param qubit Qubit for which to register the measurement result.
    @param value Boolean value of the measurement outcome (true / false = 1 / 0 respectively).
   */
  setMeasurementResult(qubit: IQubit, value: boolean) {
...
registered this result previously (see setMeasurementResult).
  ...
  @param qubit Qubit of which to get the measurement result.
...
eng.getMeasurementResult(qubit[0]) == qubit.toNumber()
   */
  getMeasurementResult(qubit: IQubit) {
  ...
  */
  ...
  Forward the list of commands to the first engine.
    @param commandList List of commands to receive (and then send on)
   */
  receive(commandList: ICommand[]): void {
