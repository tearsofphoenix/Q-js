...
import { ICommand, IEngine, IQubit, IQureg, IGate } from '@/interfaces';
...
@param engine engine which created the qubit (mostly the MainEngine)
@param gate Gate to be executed
@param qubits Array of quantum registers (to which the gate is applied)
@param controls Qubits that condition the command.
@param tags Tags associated with the command.
     */
  constructor(engine: IEngine, gate: IGate, qubits: IQureg[], controls: IQubit[] = [], tags: any[] = []) {
  ...
  copy() {
    ...
    return new Command(this.engine, this.gate.copy(), qubits as any, controlQubits, this.tags.slice(0))
  }
  ...
  orderQubits(qubits: IQureg[]): IQureg[] {
    ...
    iqi.forEach((old_positions) => {
      ...
      old_positions.forEach((v, i) => {
        orderedQubits[v] = qubitsNewOrder[i]
      })
    })
...
If we can interchange qubits 0,1 and qubits 3,4,5,
  ...
  @param nq quantum register
  */
  set controlQubits(nq: IQureg) {
...
thus early deallocation of qubits.
    ...
    @param qubits List of qubits which control this
    gate, i.e., the gate is only executed if all qubits are in state 1.
  */
  addControlQubits(qubits: IQubit[]) {
