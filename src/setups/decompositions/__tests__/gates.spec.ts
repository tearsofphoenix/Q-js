...
import r2rzandph from '@/setups/decompositions/r2rzandph'
import { DummyEngine } from '@/cengines/testengine';
import { tuple } from '@/libs/util';
import { CRz, Toffoli } from '@/ops/shortcuts';
import { Control } from '@/meta/control'
import crz2cxandrz from '@/setups/decompositions/crz2cxandrz'
import ph2r from '@/setups/decompositions/ph2r'
import toffoli2cnotandtgate from '@/setups/decompositions/toffoli2cnotandtgate'
import { IEngine, ICommand } from '@/interfaces';

const { Tdag } = Gates

describe('gates test', () => {
  const low_level_gates = (eng: IEngine, cmd: ICommand) => {
    const g = cmd.gate
  ...
  it('should test_globalphase', () => {
    const low_level_gates_noglobalphase = (eng: IEngine, cmd: ICommand) => (low_level_gates(eng, cmd)
  ...
  it('should test_gate_decompositions', () => {
    ...
    m.forEach((val: Complex, idx: number) => {
      const v2 = m2[idx]
